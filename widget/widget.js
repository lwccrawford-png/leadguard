...
(function () {
  ...
  // The embed snippet's data-* attributes are a static snapshot of whatever the
  // dashboard's "Embed on your website" tab showed at the moment someone last pasted
  // it in — they do NOT update when a business changes its name, greeting, color,
  // avatar, or suggested questions afterward, since that's just static HTML sitting on
  // the client's own site. Refreshing from the live /api/business here means a
  // dashboard save takes effect on every embed immediately, with nothing to re-paste.
  // Falls back to the static attributes above (unchanged prior behavior) if this fetch
  // is slow or fails, so the widget still renders correctly even if the client backend
  // is briefly unreachable.
  var CONFIG_FETCH_TIMEOUT_MS = 2500;
  function fetchLiveConfig(callback) {
    var done = false;
    ...
    fetch(API_BASE.replace(/\/$/, "") + "/api/business")
      .then(function (res) {
